...
def func_books(item_count, users, path):
    ...
    num_users = len(user_ids) # Books 603668
    user_map = dict(zip(user_ids, list(range(num_users))))

    total_train = export_data(path + 'UserBehavior_' + str(filter_size) + '.csv', user_ids,
                              users, user_map, item_map)
...
def func(item_count, users, path):
    ...
    print("item" + str(len(item_map)))  # 1708530
    print("user" + str(len(user_map)))  # 976777
    total_train = export_data(path+'UserBehavior_'+str(filter_len)+'_'+str(filter_size)+'.csv', user_ids, users, user_map, item_map)
    # 把item-cate文件写入：
...
def counter_taobao(item_count, users, path):
    ...
    user_map = {}
    csv_file = csv.reader(open("../data/taobao_data/UserBehavior.csv", 'r'))
...
def main():
    ...
    if name == 'book':
        ...
        func_books(item_count, users, path)
    elif name == 'taobao':
        item_count, users = read_from_taobao('../data/taobao_data/UserBehavior.csv')
        for i in range(0, 5):
            filter_size = filter_size + 5
            print("filter_size" + str(filter_size))
            # item_count, users = read_from_taobao('/home/ubuntu/jx/Data/TaobaoData/UserBehavior.csv')
            func(item_count, users, path)
            # counter_taobao(item_count, users, path)
