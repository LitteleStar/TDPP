...
class GRURec(torch.nn.Module):
    def __init__(self,n_iid, EMBEDDING_DIM, HIDDEN_SIZE, BATCH_SIZE, SEQ_LEN, MEMORY_SIZE,device=None):
        ...
        self.gru = torch.nn.ModuleList([torch.nn.GRU(self.embedding_dim, self.hidden_size, self.num_layers) for _ in range(self.interst_num)])  ##num_layer网络层数
    ...
    def output_user(self):
        ##本来是四个embdeeing (256 dim) 的列表===融合为 256 4 dim
        _= torch.cat((self.interest_em[0], self.interest_em[1],self.interest_em[2],self.interest_em[3]), -1)
        interest_= _.reshape(self.batch_size, self.interst_num, self.hidden_size)
        interest=torch.reshape(interest_,[-1, self.hidden_size])
    ...
    def forward(self, inps):
        ...
        for i in range(self.interst_num):
            ...
            self.interest_em.append(final_state)
            self.user_eb = final_state
            loss_=self.build_sampled_softmax_loss(self.user_eb)
            ##这里只做累加操作是不是不大好
            loss=loss+loss_
            #这是gru pytorch源码loss
